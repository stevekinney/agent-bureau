...
- **Provider and loop runtime**: `operative` ships provider factories (`@lostgradient/operative/anthropic`, `@lostgradient/operative/openai`, `@lostgradient/operative/gemini`), fallover, routing, structured output, cache wrappers, retries, hook expansion, guardrails, sessions, scheduler support, and enhanced streaming.
- **Durable execution**: `@lostgradient/operative/durable` provides Weft-backed crash-and-resume runs (`createRunEngine`, `reattachDurableActiveRun`), and `bureau` wires boot-time recovery (`recoverAll`) plus durable signals, updates, queries, and schedules on top.
...
- **Multi-agent orchestration**: subagent tools (`createSubagentTool`), handoffs (`createHandoffTool`), a supervisor (`createSupervisor`), and a typed `AgentDefinitions` catalog (`bureau.agents`, `bureau.run(name, input)`) let one run delegate to or hand off to another agent.
...
- **Composition**: `bureau` (`createBureau()`) assembles providers, tools, memory, skills, session persistence, durable execution, guardrails, and multi-agent behavior from one configuration surface, plus a durable audit trail.
- **State, skills, memory, and storage**: `@lostgradient/operative/store`, `skills`, and `memory` cover run tracking, skill loading, and hybrid (BM25 + embedding) memory ranking; key-value persistence is backed by Weft's durable storage (`@lostgradient/weft`).
