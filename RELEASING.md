...
3. Open your PR. `mirror-verify.yaml` (generated by the corvidae mirror transform; there is no
   `ci.yml` in this repository) builds and typechecks the publishable packages, packs them, and
   runs publint/attw against the tarballs. A change to a foundation package
   (`@lostgradient/lifecycle`, `@lostgradient/tool-protocol`, `@lostgradient/cryptography`,
   `@lostgradient/embeddings`) that affects a product's output needs its own changeset if that
   foundation package's own version changed, in addition to any changeset the product needs — they
   are real, separately versioned dependencies now, not inlined into the product's build output.
...
2. Add the package to the release pipeline: remove it from the `ignore` list in
   ...
   `scripts/release.ts`, in dependency order relative to the packages already there. Add the
   package's `packageName` to the `--filter` list in `release.yml`'s "Build publishable packages"
   step (there is no `ci.yml` in this repository). Make sure its build produces self-contained
   artifacts (the package-shape gate enforces this).
...
- **`.changeset/config.json`** — `access: public`, `baseBranch: main`, and an `ignore` list of every
  package except the publishable ones. `changeset version` never touches ignored packages, but —
  verified directly by running it — it does **not** rewrite an internal `workspace:*` dependency to
  a concrete version either, for a package it *is* versioning. That rewrite happens later, in
  `scripts/release.ts`, immediately before each package publishes (see below), not here.
- **`.github/workflows/release.yml`** — runs on push to `main`. Pins `npm@^11` (trusted publishing
  ...
  (There is no `ci.yml` in this repository — it was retired; `mirror-verify.yaml`, generated by the
  corvidae mirror transform, is what runs build + typecheck + pack + publint/attw on pull requests.)
- **`scripts/release.ts`** — exports `RELEASE_INVENTORY`, the explicit list of publishable packages
  ...
  versions; for a package about to publish, rewrites any `workspace:`-protocol dependency in
  `dependencies`/`peerDependencies`/`optionalDependencies` to that dependency's concrete current
  version, in place, on disk (never committed — this runs only in the release workflow's own CI
  checkout); runs the shape gate against that rewritten manifest, which refuses a remaining
  `workspace:`/`catalog:` specifier and refuses a dependency on another workspace package at a
  version that isn't actually on the registry or published earlier in the same run; and then
  publishes with `npm publish --provenance --access public --ignore-scripts` from the package
  directory (provenance requires publishing from a directory, not a prebuilt tarball). On partial
  failure it stops without unpublishing; re-running
  is safe.
