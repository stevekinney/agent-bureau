...
1. **Pure refactor of `loop.ts` (no behavior change).** Extract the per-step body at the `stepToolbox.execute` boundary (`loop.ts:769`) into two **snapshot-in / snapshot-out** halves: `runStepUpToTools(deps, conversationSnapshot, cursor) -> { generated, conversationSnapshot }` and `applyToolResults(deps, conversationSnapshot, generated, toolResults) -> conversationSnapshot`. Each rehydrates `Conversation.from(snapshot)` internally and returns a fresh snapshot — no `Conversation` instance leaves a half. Re-express `executeLoop()` in terms of both halves (it can keep a long-lived instance in-process; only the *halves'* signatures are snapshot-based, so the durable generator reuses them verbatim). **Check:** the full existing operative test suite passes unchanged (`turbo run test --filter=operative`). This is the anti-duplication foundation — the durable generator calls the *same* halves.
...
4. **A `recoverAll` per-handle recovery hook** (seam 5). A callback invoked per recovered `WorkflowHandle` on boot would let consumers do both halves of recovery without polling: re-inject the run's non-serializable deps (`generate`/`toolbox`/`hooks`) so the resumed generator can advance, *and* re-attach their observable surface (our ActiveRun adapter + `store.register`). The deps-re-injection need is the load-bearing one — without it a recovered run cannot make progress at all.
