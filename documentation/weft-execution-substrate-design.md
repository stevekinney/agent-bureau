...
**Consumer blast radius, pass 1:** gateway / operative store / herald change **0 files** — the `ActiveRun` contract is preserved verbatim under inline mode.
...
>
> **Accepted serialization-boundary divergence (object identity, not value):** a live error object cannot cross a checkpoint, so the durable path carries error MESSAGES, not error instances. `RunResult.error` (terminal errors) and `RunResult.schemaValidation.error` (a `responseSchema` failure, usually a `ZodError` with structured `.issues`) are reconstructed as plain `Error(message)` on the durable path — their `instanceof` class is preserved for the known operative error subclasses (so `finishReason` classification is identical), but `.issues`/`.stack`/`.cause` are reduced to the message. The load-bearing fields (`finishReason`, `schemaValidation.success`) are exact. This is the same structural-equivalence-not-identity contract already adopted for conversations (snapshots, not `Conversation` instances). operative does not own the user's schema library and cannot faithfully reconstruct an arbitrary validation error type, so this is accepted rather than papered over.
>
...
9. **Route `run()` / `createRun()` through the adapter** when an engine is present; legacy in-memory path otherwise. **Check:** gateway / operative store / herald compile and their suites pass with **0 file changes**.
