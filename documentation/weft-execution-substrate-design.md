...
> Phase 5 made Weft durable execution the **default** execution model. A normal `createRun()`/`run()` with an engine present is checkpointed and resumes from its last completed step after a crash — proven cross-process (two bureaus on one SQLite file; bureau A crashes mid-step, bureau B reconstructs deps from config and resumes via `recoverAll`, re-running only the in-flight step).
>
...
> - **5c** routed `createRun`/`run` through a durable `ActiveRun` adapter when an engine is present (optional 2nd arg, mirroring `executeLoop(options, emitter?)`); gateway wires it by default. Closed seam #7 (run-level event parity) — a durable run fires `run.completed` so gateway's `store.register` sees it.
...
>
> Still-open seams (degrade only the resume window, never the happy path): #1 durable in-step retry counters, #3 compaction-as-activity durability, #11 hook re-emit-on-resume classification, #4 cross-crash tool dedup (Tier-0 upstream), #10 in-process streaming, #5b ActiveRun reconstruction for recovered-run live visibility (session status IS persisted). Default-on with storage; storageless runs use the in-memory loop (necessary, not a gap).
>
