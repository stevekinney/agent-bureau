...
1. **Pure refactor of `loop.ts` (no behavior change).** Extract the per-step body at the `stepToolbox.execute` boundary (`loop.ts:769`) into two **LIVE-INSTANCE** halves: `runStepUpToTools(deps, conversation, cursor) -> { generated, ... }` and `applyToolResults(deps, conversation, generated, toolResults) -> void`. Each takes a **live `Conversation` instance** and mutates it in place, exactly as the inline code does today.
   ...
   Re-express `executeLoop()` in terms of both halves. **Check:** the full existing operative test suite passes unchanged (`turbo run test --filter=operative`). This is the anti-duplication foundation — the durable generator calls the *same* halves.
