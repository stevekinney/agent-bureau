...
| :-------------------------------------------------------------------- | :--------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Agent run (`createAgent`/`createLazyAgent`/`bureau.run`)              | Independently owned                                                    | Asynchronous                                                                                                                 | Process-local; durable when started through a durable route             | `runId`                                                                                                       | Live `AgentRun`, synchronous, non-thenable; durable projection via `Bureau.getRun(id)`                                                                                                                                                                                                                                                                                                                                  | This amendment (snapshot/subscribeSnapshot); AB-88 (liveness fields)                                                                                                                                                                                 |
...
| Session                                                               | Independently owned                                                    | Asynchronous                                                                                                                 | Durable when a session store is composed                                | `sessionId`                                                                                                   | **Declared non-conformance.** `Bureau.getSession`/`listSessions` give discovery, but the shipped `SessionHandle` (`packages/operative/src/session/session-handle.ts:107`) has no `snapshot()`, no `subscribeSnapshot()`, no terminal `result()`, and no cleanup acknowledgement. A session cannot today satisfy the inspect, observe, await, and cleanup guarantees this contract requires of independently owned work. | AB-88 owns extending the observation surface to sessions                                                                                                                                                                                             |
...
| Durable background operation (`BureauOptions.durableBackgroundTasks`) | Independently owned                                                    | Asynchronous                                                                                                                 | Durable                                                                 | shares the durable-run identity family                                                                        | `getDurableRun`/`listDurableRuns`, not a distinct resource kind                                                                                                                                                                                                                                                                                                                                                         | Unchanged                                                                                                                                                                                                                                            |
...
if (schedule) {
  ...
  if (inFlight) {
    ...
    bureau.abortRun(inFlight.runId); // idempotent once AB-37 lands its remediation
  }
}
```
