...
| Session                                                                               | Independently owned                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Asynchronous                                                                                                                                                                                                                                                                                 | Durable **only when the session store's backing storage is persistent**. A `SessionStore` over `MemoryStorage` is process-local: `BureauOptions` documents that memory storage loses checkpoints with the process, and the repository composes valid session stores over it extensively.                                                                                                                                                                                                                                  | `sessionId`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **Declared non-conformance, narrowed by AB-214.** `Bureau.getSession`/`listSessions` give discovery. `SessionHandle.run()`'s returned `AgentRun` now proxies `snapshot()`/`subscribeSnapshot()` to its inner `ActiveRun` once one exists (a synthetic `'created'` snapshot before that) — a mechanical AB-214 fix, not the dedicated session-level `LivenessObservable` wiring AB-88's own implementation-slices list assigns to obs-02, which remains outstanding. Still absent: no terminal result, no child discovery over the runs it owns, no scoped child cancellation, and no cleanup acknowledgement. A session cannot today satisfy the inspect, observe, await, and cleanup guarantees this contract requires of independently owned work.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | AB-88 owns extending the observation surface to sessions, and must not promise reattachment a memory-backed store cannot provide; **AB-37 owns the cleanup acknowledgement**, which [Required capabilities](#required-capabilities) assigns to it. Folding cleanup into the observation owner would leave part of this row's declared non-conformance unowned once AB-88 lands; **AB-50** owns child discovery and scoped child cancellation over the runs, session-input records, and steering commands a session owns (session input added by **AB-42**, steering commands added by **AB-67**) |
