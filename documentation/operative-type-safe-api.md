...
| Gateway hook run with an idempotency key (`POST /hooks/*`)                            | Independently owned                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Asynchronous                                                                                                                                                                                                                                                                                 | **Process-local key state.** `idempotencyKeys` is an in-process `Set` (`routes/hooks.ts:26`), so no key survives a restart and two gateway instances share none of it. The run it guards is classified in row 1.                                                                                                                                                                                                                                                                                                          | The caller's `Idempotency-Key`, plus the `runId` on the returned summary                                                                                                                                                                                                                                                                                                                                                                                                                             | **Declared non-conformance, four ways.** The key is not scoped: `idempotencyKeys` holds the raw header value, while the vocabulary above scopes a key to a principal _and_ an operation kind, so two principals sending the same string collide and the second is refused as a duplicate of work it never started. A repeated key returns `409` (`routes/hooks.ts:44-52`) instead of the original receipt, so a caller retrying a request it is unsure landed learns only that it already asked. The key is released **only in the `catch`** (`:88-89`), so a successful call's key is held for the process's lifetime while a failed one is freed at once — the reverse of the case a retry needs. Retention is neither bounded nor tied to locator retention.                                                                                                                                                                                                                                                                                                                                                                                        | **AB-185** owns returning the original receipt, distinguishing a conflicting reuse, scoping the key to a principal and operation kind, and stating the retention it keeps                                                                                                                                                                                                                                                                                                                                                  |
...
**Idempotency-key request and receipt shapes are unowned _for operative-level
start operations_.** None of them accepts a key today. This contract fixes the
semantics any key-accepting operation must satisfy, so the first one cannot
invent different rules, but choosing which of those operations take a key and
what carries it is out of scope here.

This is deliberately narrower than it used to read. The gateway route
`POST /hooks/*` already accepts an `Idempotency-Key` and is classified in the
table as a declared non-conformance owned by AB-185, so a flat claim that no
start operation accepts a key would contradict a decision this document has
already made two sections earlier.
