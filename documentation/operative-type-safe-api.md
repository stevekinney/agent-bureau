...
| :------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Agent run (`createAgent`/`createLazyAgent`/`bureau.run`)       | Independently owned                                                                                                                                                                                                                                                                                                                                                                                                                    | Asynchronous                                                                                                                 | Process-local; durable when started through a durable route                                                                                                                                                                                                                                                                                                                      | `runId`                                                                                   | Live `AgentRun`. For durable runs the engine-backed locator is `Bureau.getDurableRun(id)` (`create-bureau.ts:2999`), **not** `Bureau.getRun(id)`, which reads only the process-local `Store` (`create-bureau.ts:2894`) and can return `undefined` after a restart while the engine still holds the run. Retained terminal records in the process-local store are not made durable by boot recovery.                                                                                                   | This amendment; AB-88 (liveness fields, and closing the retained-result gap between the two locators)                                                                                                                                                                                                                                                                                                                                                                   |
...
| Background webhook delivery (`Bureau.webhookNotifier`)         | Parent-owned by the notifier, and through it by the Bureau — `flush()` awaits active deliveries                                                                                                                                                                                                                                                                                                                                        | Asynchronous — `notify()` launches tracked `deliver()` promises (`webhook-notifier.ts:128`)                                  | **Conditional on a KV store.** With `runtime.kv` present, delivery records persist and are inspectable via `listDeliveries()`. Without it — a supported configuration — `persist()` returns early (`webhook-notifier.ts:278`), no record is written, and there is no durable evidence or discoverable id at all: the attempt is purely process-local.                            | Delivery record id via `listDeliveries()` **when KV is configured**; none otherwise       | **Declared non-conformance.** Even with KV, `listDeliveries()` plus `flush()` gives only discovery and a bulk drain: no caller can inspect, await, or cancel one delivery, and there is no per-delivery cleanup acknowledgement. Without KV there is no locator to speak of.                                                                                                                                                                                                                          | AB-88 owns giving these an observable surface, and must gate any restart or inspection guarantee on KV being present                                                                                                                                                                                                                                                                                                                                                    |
