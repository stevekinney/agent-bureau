...
**One shipped path already accepts one, and does not satisfy these semantics.** The mounted gateway route `POST /hooks/*` reads an `Idempotency-Key` header and starts a Bureau run (`packages/gateway/src/routes/hooks.ts:44-52`, `:82`). It is classified below as a declared non-conformance rather than left out of the table: an earlier revision of this section asserted that no start operation accepted a key at all, which was false and would have let a shipped breach pass as an undecided question.
...
| Gateway hook run with an idempotency key (`POST /hooks/*`)                            | Independently owned                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Asynchronous                                                                                                                                                                                                                                                                                 | **Process-local key state.** `idempotencyKeys` is an in-process `Set` (`routes/hooks.ts:26`), so no key survives a restart and two gateway instances share none of it. The run it guards is classified in row 1.                                                                                                                                                                                                                                                                                                          | The caller's `Idempotency-Key`, plus the `runId` on the returned summary                                                                                                                                                                                                                                                                                                                                                                                                                             | **Declared non-conformance, three ways.** A repeated key returns `409` (`routes/hooks.ts:44-52`) instead of the original receipt, so a caller retrying a request it is unsure landed learns only that it already asked. The key is released **only in the `catch`** (`:88-89`), so a successful call's key is held for the process's lifetime while a failed one is freed at once — the reverse of the case a retry needs. Retention is neither bounded nor tied to locator retention.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | **AB-185** owns returning the original receipt, distinguishing a conflicting reuse, and stating the retention it keeps                                                                                                                                                                                                                                                                                                                                                                                                     |
