...
| Scheduler submission (`submitSchedulerTask`)                   | Independently owned, detached                                          | Asynchronous                                                                                                                 | Process-local — Bureau's in-process flow-control queue, not Weft-backed                                                                                                                                                                                                                                                                                                          | `taskId`                                                                                                      | **Declared non-conformance.** The receipt carries `taskId`, but `create-bureau.ts:2869` discards the scheduler's result promise and the public `Scheduler` surface exposes only transient `getState()` and `cancel()`. Once a task leaves the active or queued state its result cannot be retrieved, awaited, or its cleanup confirmed by that id.                                                                 | AB-180 owns closing it                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Individual schedule fire                                       | Independently owned, separate identity from its definition             | Asynchronous                                                                                                                 | Durable                                                                                                                                                                                                                                                                                                                                                                          | `runId`                                                                                                       | An ordinary run (see row 1); the definition holds a reference list, never the reverse                                                                                                                                                                                                                                                                                                                              | AB-41                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Evaluation suite (`runEvaluationSuite`, as shipped today)      | Synchronous inline — returned directly to its caller, never reattached | Asynchronous — it is an `async` function awaiting cases concurrently (`packages/evaluation/src/run-evaluation-suite.ts:126`) | Process-local                                                                                                                                                                                                                                                                                                                                                                    | none                                                                                                          | Caller holds the awaited result directly; there is no locator because there is nothing to reattach to                                                                                                                                                                                                                                                                                                              | Unchanged. The two axes genuinely differ here, which is why they are separate columns: _synchronous inline_ is an ownership statement, not a claim that the function is synchronous.                                                                                                                                                                                                                                                                                    |
| Background judge evaluation (`online-evals`, as shipped today) | Parent-owned by the bureau that launched it                            | Asynchronous                                                                                                                 | Process-local                                                                                                                                                                                                                                                                                                                                                                    | none of its own — tracked in an in-memory `activeEvaluations` set (`packages/bureau/src/online-evals.ts:175`) | **Declared non-conformance.** `flush()` drains all in-flight evaluations, but no caller can discover, inspect, await, or cancel one individually.                                                                                                                                                                                                                                                                  | AB-88 owns giving these an observable surface                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
if (schedule) {
  ...
  if (inFlight) {
    // Synchronous: this requests and accepts the cancellation. Awaiting the durable
    // acknowledgement is the separate path described in Durable cancellation.
    bureau.abortRun(inFlight.id); // idempotent once AB-37 lands its remediation
  }
}
```
