...
| :------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Agent run (`createAgent`/`createLazyAgent`/`bureau.run`)       | Independently owned                                                                                                                                                                                                                                                                                                                                                                                                                    | Asynchronous                                                                                                                 | Process-local, unless started through a durable route **and** that engine's checkpoint store persists. `durableExecution: true` over a memory backend still uses the durable engine, and its checkpoints do not survive the process.                                                                                                                                             | `runId`                                                                                   | Live `AgentRun`. For durable runs the engine-backed locator is `Bureau.getDurableRun(id)` (`create-bureau.ts:2999`), **not** `Bureau.getRun(id)`, which reads only the process-local `Store` (`create-bureau.ts:2894`) and can return `undefined` after a restart while the engine still holds the run.                                                                                                                                                                                                                                                                      | This amendment; AB-88 (liveness fields, and closing the retained-result gap between the two locators)                                                                                                                                                                                                                                                                                                                                                                   |
| Diagnostic run (`createDiagnosticAgentRun`)                    | Independently owned                                                                                                                                                                                                                                                                                                                                                                                                                    | Asynchronous                                                                                                                 | **Whatever the wrapped run is.** Not durable by construction: `createDiagnosticAgentRun(activeRun)` (`packages/operative/src/agent-run.ts:389`) is publicly exported, accepts any `ActiveRun`, and performs no origin check, so a consumer can wrap a process-local run created without durable routing. Durable only when the run it wraps was started through a durable route. | `runId`                                                                                   | Live `DiagnosticAgentRun`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | This amendment; AB-88, which must derive restart and durable-cancellation guarantees from the wrapped run rather than from the wrapper                                                                                                                                                                                                                                                                                                                                  |
...
| Child run (subagent delegation)                                | Parent-owned by default                                                                                                                                                                                                                                                                                                                                                                                                                | Asynchronous                                                                                                                 | Inherits its parent's durability                                                                                                                                                                                                                                                                                                                                                 | `childRunId`, `parentRunId`                                                               | Discovered through the parent's ownership graph; scoped abort through the parent, not a standalone public locator, unless the owner explicitly returns a live child handle                                                                                                                                                                                                                                                                                                                                                                                                   | AB-50                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Recurring schedule definition                                  | Independently owned                                                                                                                                                                                                                                                                                                                                                                                                                    | Asynchronous                                                                                                                 | A native Weft schedule, durable **only when the engine's checkpoint store persists** — over a memory backend the definition dies with the process                                                                                                                                                                                                                                | `scheduleId`                                                                              | Locator via `createSchedule`/`getSchedule`/`listSchedules`. **Declared non-conformance:** `createSchedule` returns `Promise<ScheduleSummary \| null \| undefined>` and yields `null` when the schedule registers but its summary cannot be retrieved, so independently owned work exists with no locator returned to its caller.                                                                                                                                                                                                                                             | AB-41, which must return the identifier even when description retrieval fails                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Background judge evaluation (`online-evals`, as shipped today) | **Parent-owned while the Bureau lives, detached the moment it is disposed.** `OnlineEvalSampler.dispose()` (`online-evals.ts:257-260`) sets a flag and removes its event listener; it neither cancels nor awaits `activeEvaluations`, so a slow judge keeps running after its owner is gone. Under this contract's control-based definition of detachment, that work is detached — nothing can still cancel it.                        | Asynchronous                                                                                                                 | Process-local                                                                                                                                                                                                                                                                                                                                                                    | none of its own — tracked in an in-memory `activeEvaluations` set (`online-evals.ts:175`) | **Declared non-conformance.** `flush()` drains what is in flight, but no caller can discover, inspect, await, or cancel one evaluation, and disposal silently orphans any still running.                                                                                                                                                                                                                                                                                                                                                                                     | AB-88 for the observable surface; the disposal gap belongs with AB-37, which owns awaitable shutdown                                                                                                                                                                                                                                                                                                                                                                    |
...
| Background webhook delivery (`Bureau.webhookNotifier`)         | Parent-owned by the notifier, and through it by the Bureau — `flush()` awaits active deliveries                                                                                                                                                                                                                                                                                                                                        | Asynchronous — `notify()` launches tracked `deliver()` promises (`webhook-notifier.ts:128`)                                  | **Conditional on the KV backend persisting**, not on KV being configured. Without `runtime.kv`, `persist()` returns early (`webhook-notifier.ts:278`) and no record is written at all. With a KV over `MemoryStorage` — a supported configuration — records are written and `listDeliveries()` works, but only until the process exits.                                          | Delivery record id via `listDeliveries()`, for as long as the backend retains it          | **Declared non-conformance.** Even with a persistent backend, `listDeliveries()` plus `flush()` gives only discovery and a bulk drain: no caller can inspect, await, or cancel one delivery, and there is no per-delivery cleanup acknowledgement.                                                                                                                                                                                                                                                                                                                           | AB-88, which must gate any restart or reattachment guarantee on the backing store persisting rather than on KV being present                                                                                                                                                                                                                                                                                                                                            |
...
- **Authorization-selected**, which is what `AgentRun` and Bureau locators use — a single `snapshot()` whose returned projection is determined by the calling principal that Bureau already carries on `BureauRunOptions`. Bureau owns authorization in this system, so the principal is available at the boundary without a second accessor.
