...
| Background webhook delivery (`Bureau.webhookNotifier`)         | Parent-owned by the notifier, and through it by the Bureau — `flush()` awaits active deliveries, so the owner controls their lifecycle                                                                                                                                                                                                                                                                                                 | Asynchronous — `notify()` launches tracked `deliver()` promises (`packages/bureau/src/webhook-notifier.ts:128`)              | Delivery _records_ are durable and inspectable via `listDeliveries()`; the in-flight delivery attempt is process-local                                                                                                                                                                                                                                                           | Delivery record id, via `listDeliveries()`                                                                    | **Declared non-conformance.** `listDeliveries()` gives discovery of persisted records and `flush()` awaits everything in flight, but no caller can inspect, await, or cancel one delivery individually, and there is no per-delivery cleanup acknowledgement.                                                                                                                                                                                                                                         | AB-88 owns giving these an observable surface, alongside background judge evaluations                                                                                                                                                                                                                                                                                                                                                                                   |
