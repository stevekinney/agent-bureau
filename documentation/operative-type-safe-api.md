...
| Background webhook delivery (`Bureau.webhookNotifier`)         | **Parent-owned while the Bureau lives, detached the moment it is disposed.** `WebhookNotifier.dispose()` (`webhook-notifier.ts:470-473`) sets a flag and removes its listener; an in-flight `fetchImpl` receives no abort signal and `deliver()` keeps awaiting it, so the delivery outlives the owner with nothing able to cancel it — the same shape as background judge evaluations.                                                | Asynchronous — `notify()` launches tracked `deliver()` promises (`webhook-notifier.ts:128`)                                  | **The record and the work differ, and only the record can be durable.** The _record_ persists when the KV backend persists; without `runtime.kv`, `persist()` returns early (`:278`) and nothing is written, and over `MemoryStorage` it dies with the process. The _delivery work_ is always process-local: startup never scans pending records and `deliver()` returns immediately when the key exists (`:312-315`), so a process that exits after persisting `status: 'pending'` leaves a durable record for work that will never resume or be retried. | Delivery record id via `listDeliveries()`, for as long as the backend retains it          | **Declared non-conformance, three ways.** No caller can inspect, await, or cancel one delivery; no per-delivery cleanup acknowledgement; and a persisted `pending` record can permanently outlive its stopped work.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | AB-88 for the observable surface and for gating any restart guarantee on the backend; **AB-37 for the disposal gap**, alongside the identical one on judge evaluations                                                                                                                                                                                                                                                                                                  |
