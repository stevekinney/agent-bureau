...
| Child run (subagent delegation)                                                       | Parent-owned by default                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Asynchronous                                                                                                                                                                                                                                                                                 | **From the child's own resolved route and backing store, not inherited.** `createSubagentTool`'s `agent: RunnableAgent` (AB-19) is arbitrary and `parentContext.durable` is caller-supplied event metadata that neither selects nor validates the child's execution route, so a durable parent can invoke a process-local child and the reverse.                                                                                                                                                                          | `childRunId`, `parentRunId`                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Discovered through the parent's ownership graph; scoped abort through the parent, not a standalone public locator, unless the owner explicitly returns a live child handle                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | **AB-50**, which must derive the child's durability from its actual route rather than the parent's, or constrain the factory so inheritance is enforced                                                                                                                                                                                                                                                                                                                                                                                                                                          |
 ...
 *  both fields, or neither, is rejected by the type checker at compile time. The
 *  runtime admission check that exactly one is present stays as defense in
 *  depth. */
...
These seven exported types carry no runtime behavior on their own. `submitSteeringCommand`, the `runStep` boundary read, and `GenerateContext` threading are implemented by the issues that consume this contract, not by this section.
