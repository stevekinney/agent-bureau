...
| Recurring schedule definition                                  | Independently owned                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Asynchronous                                                                                                                 | A native Weft schedule, durable **only when the engine's checkpoint store persists** — over a memory backend the definition dies with the process                                                                                                                                                                                                                                                                                                                                                                         | `scheduleId`                                                                                           | Locator via `createSchedule`/`getSchedule`/`listSchedules`. **Declared non-conformance, two paths.** `Bureau.createSchedule` returns `Promise<ScheduleSummary \| null \| undefined>` and yields `null` when the schedule registers but its summary cannot be retrieved, leaving independently owned work with no locator. Separately, the exported `createAgentSchedule` / `createAgentScheduler().schedule` return an `AgentScheduleHandle` exposing only `id`, `pause`, `resume`, `cancel`, and `describe` — no snapshot, no observation, no cleanup acknowledgement — so the direct public API is outside this contract even once the Bureau method is fixed.                                          | **AB-41** for both: return the identifier when description retrieval fails, and bring `AgentScheduleHandle` up to the required capabilities                                                                                                                                                                                                                                                                                                                             |
...
| Evaluation suite (`runEvaluationSuite`, as shipped today)      | Inline — returned directly to its caller, never reattached                                                                                                                                                                                                                                                                                                                                                                                                                                      | Asynchronous — it is an `async` function awaiting cases concurrently (`packages/evaluation/src/run-evaluation-suite.ts:126`) | Process-local                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | none                                                                                                   | Caller holds the awaited result directly; there is no locator because there is nothing to reattach to                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Unchanged. This row is exactly why the axes are separate columns: ownership is `inline` while execution is asynchronous.                                                                                                                                                                                                                                                                                                                                                |
