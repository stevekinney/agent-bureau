...
The same asynchronous-acknowledgement discipline [Durable cancellation](#durable-cancellation) states for the stop side of a durable operation applies to the start side: an acknowledged durable run must be durable, so a caller that receives a run identifier and then crashes can recover it at any later point.

**AB-361 implements this for `bureau.createRun`.** `ActiveRun` gained an optional `durablyStarted?: Promise<void>` (`operative/src/create-run.ts`): on the durable branch, `createDurableActiveRun` (`operative/src/durable/active-run-adapter.ts`) resolves it the moment `context.engine.start(...)` commits this run's initial workflow record — the same write Weft's own boot-time `recoverAll()` depends on to find it — never when `result` (the run's own completion) settles. `bureau.createRunFromRequest` awaits it, immediately before returning a run identifier to its caller, so `createRun`'s own promise cannot resolve until that record is committed: a process killed any time after the caller holds the identifier is recoverable. `reattachDurableActiveRun`'s `durablyStarted` is already resolved — a recovered run's record predates the reattaching process — and the in-memory branch leaves the field `undefined`, since there is no durable write to await and no caller reads it. `durablyStarted` rejects on any pre-commit failure, not only `engine.start` itself rejecting: a caller-supplied `onServices` callback throwing before `engine.start` is ever reached settles it the same way (review fix, PRRT_kwDORvupsc6gWc39/PRRT_kwDORvupsc6gWb3a). Either failure is routed through the same `makeErrorResult` helper the pre-existing `onRunStart`-failure branch already used, so `ActiveRun.result` still resolves with a `finishReason: 'error'` `RunResult` (dispatching a genuine `run.completed`/`RunCompletedEvent`) rather than rejecting raw — the terminal event `createRunFromRequest`'s catch block relies on to unregister the run and persist its terminal session state still fires, even though `durablyStarted` itself rejected. This is the START-side counterpart the previous paragraph's guarantee named but left unimplemented at decision time; `bureau.run`'s catalog-dispatch branch (`RunnableAgent.run()`) is unaffected — AB-34's non-thenable, synchronous-return requirement for an in-process start means it has no "resolves" moment analogous to `createRun`'s promise to gate in the first place.
...
`createSubagentTool` (AB-300) closes the automatic-wiring gap this section originally left open: it reads the parent run's own `delegatedAuthority` off its per-execution `ToolContext.executionContext` — threaded there from `AgentRunContext.delegatedAuthority` through `RunOptions.delegatedAuthority`, matching the AB-233 `childRegistry`/`parentRunId` pattern — attenuates it with the tool's own `delegatedAuthority` construction option (a per-tool narrowing for the specific child it dispatches, if any) via `attenuateDelegatedAuthority`, and forwards the composed result into its own `dispatchChildRun` call. A parent run carrying no grant and a tool with no narrowing of its own dispatch with `delegatedAuthority` left `undefined`, unchanged from before this wiring existed. A caller driving `dispatchChildRun` directly (bypassing `createSubagentTool`) still supplies `delegatedAuthority` explicitly, exactly as before.
