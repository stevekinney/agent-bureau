...
import { SharedMemoryManager } from './shared-memory.ts';
...
export class WorkerPool {
  ...
  constructor(config: PoolConfig = {}) {
    ...
    this.workerScript = config.workerScript || new URL('./vector-worker.ts', import.meta.url).href;
  ...
  > {
    ...
    const filteredVectors = filter ? vectors.filter((v) => filter(v.metadata || {})) : vectors;
  ...
  async normalizeVectors(vectors: Float32Array[]): Promise<Float32Array[]> {
    ...
    const promises = chunks.map((chunk) => this.execute('vector_normalize', { vectors: chunk }));
