...
import { createDistanceCalculator } from '../search/distance-metrics.ts';
...
function distanceToScore(distance: number, metric: DistanceMetric, vectorLength?: number): number {
  switch (metric) {
    case 'cosine':
...
async function performSharedSimilaritySearch(data: {
  ...
  const { sharedBuffer, dimension, queryVector, queryOffset, k, metric, startIdx, endIdx, layout } =
    data;
