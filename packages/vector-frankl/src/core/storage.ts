...
import { VectorDatabase } from './database.ts';
import { BatchOperationError, TransactionError, VectorNotFoundError } from './errors.ts';
import type { BatchOptions, BatchProgress, StorageAdapter, VectorData } from './types.ts';
...
export class VectorStorage implements StorageAdapter {
   ...
   */
  private async putVectorInStore(store: IDBObjectStore, vector: VectorData): Promise<void> {
    return new Promise((resolve, reject) => {
      const vectorToStore = {
        ...vector,
