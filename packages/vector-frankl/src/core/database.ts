...
import { BrowserSupportError, DatabaseInitializationError, TransactionError } from './errors.ts';
...
export class VectorDatabase {
  ...
  private async openDatabase(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        reject(
          new DatabaseInitializationError('Failed to open database', request.error || undefined),
        );
      };
      ...
      request.onblocked = () => {
        reject(new DatabaseInitializationError('Database upgrade blocked by other connections'));
      };
    });
  }
  ...
  private createSchema(database: IDBDatabase, _oldVersion: number): void {
    ...
    if (!database.objectStoreNames.contains(VectorDatabase.STORES.VECTORS)) {
      ...
      vectorStore.createIndex(VectorDatabase.VECTOR_INDICES.LAST_ACCESSED, 'lastAccessed', {
        unique: false,
      });
    }
  ...
  async executeTransaction<T>(
    ...
    return new Promise((resolve, reject) => {
      ...
      transaction.onerror = () => {
        reject(
          new TransactionError('execute', 'Transaction failed', transaction.error || undefined),
        );
      };
  ...
  async delete(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        reject(
          new DatabaseInitializationError('Failed to delete database', request.error || undefined),
        );
      };

      request.onblocked = () => {
        reject(new DatabaseInitializationError('Database deletion blocked by other connections'));
      };
    });
  }
