import type { DistanceMetric, VectorData } from '@/core/types.ts';

import { createDistanceCalculator, DistanceCalculator } from './distance-metrics.ts';
...
export class HNSWIndex {
  ...
  constructor(distanceMetric: DistanceMetric = 'cosine', config: Partial<HNSWConfig> = {}) {
  ...
  async addVector(vectorData: VectorData): Promise<void> {
    ...
    for (
      ...
      const searchResults = this.searchLevel(node.vector, currentClosest, 1, currentLevel);
   ...
   */
  private selectConnections(queryVector: Float32Array, candidates: string[], m: number): string[] {
