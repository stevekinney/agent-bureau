  ...
  > {
    const result = await this.database.executeTransaction(
      ...
      async (transaction) => {
        ...
        return new Promise<Array<{ id: string; data: SerializableHNSWIndex; timestamp: number }>>(
          (resolve, reject) => {
...
export class IndexCache {
   ...
   * Put index in cache
   */
  putInCache(indexId: string, index: HNSWIndex, distanceMetric: string, isDirty = false): void {
