import type { MetadataFilter } from '@/core/types.ts';
...
export class MetadataFilterCompiler {
  ...
  static compile(filter: MetadataFilter): (metadata: Record<string, unknown>) => boolean {
    ...
    if (!hasTopLevelOperators && !hasFieldLevelOperators) {
      ...
      return (metadata) =>
        MetadataFilterCompiler.matchSimpleFilter(metadata, filter as Record<string, unknown>);
    }
    ...
    for (const [key, condition] of Object.entries(filter)) {
      switch (key) {
        case '$and': {
          ...
          if (
            !conditions.every((cond) => MetadataFilterCompiler.matchComplexFilter(metadata, cond))
          ) {
            return false;
          }
        ...
        case '$or': {
          ...
          const conditions = condition as MetadataFilter[];
          if (
            !conditions.some((cond) => MetadataFilterCompiler.matchComplexFilter(metadata, cond))
          ) {
            return false;
          }
  ...
  private static matchValue(a: unknown, b: unknown): boolean {
    ...
    if (typeof a === 'object' && typeof b === 'object') {
      ...
      return aKeys.every(
        (key) => bKeys.includes(key) && MetadataFilterCompiler.matchValue(aObj[key], bObj[key]),
      );
    }
