...
describe('NamespaceRegistry', () => {
  ...
  describe('register', () => {
    ...
    it('should reject duplicate namespace names', async () => {
      ...
      expect(registry.register('duplicate', config)).rejects.toThrow(NamespaceExistsError);
    });
  ...
  describe('updateStats', () => {
    ...
    it('should throw for non-existent namespace', async () => {
      expect(registry.updateStats('does-not-exist', { vectorCount: 100 })).rejects.toThrow(
  ...
  describe('unregister', () => {
    ...
    it('should throw for non-existent namespace', async () => {
      expect(registry.unregister('does-not-exist')).rejects.toThrow(NamespaceNotFoundError);
    });
  });
  ...
  describe('findByPattern', () => {
    ...
    it('should find namespaces by regex pattern', async () => {
      ...
      expect(results).toHaveLength(2);
      expect(results.map((ns) => ns.name).sort()).toEqual(['documents-guide', 'documents-manual']);
    });
