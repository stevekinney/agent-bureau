...
import { VectorDB } from '@/api/database.ts';
import type { DistanceMetric, VectorFormat } from '@/core/types.ts';
import { debugMethod } from '@/debug/hooks.ts';
import { VectorFormatHandler } from '@/vectors/formats.ts';
...
export class BenchmarkSuite {
  ...
  private async benchmarkVectorFormats(): Promise<void> {
    ...
    const formats: Array<{ name: string; generator: (dim: number) => VectorFormat }> = [
      ...
      {
        name: 'Int8Array',
        generator: (dim) => new Int8Array(dim).map(() => Math.floor(Math.random() * 256) - 128),
      },
      {
        name: 'Uint8Array',
        generator: (dim) => new Uint8Array(dim).map(() => Math.floor(Math.random() * 256)),
      },
    ];
  ...
  private generateRecommendations(results: BenchmarkResult[]): string[] {
    ...
    if (searchResults.length > 0) {
      const avgSearchOps =
        searchResults.reduce((sum, r) => sum + r.operationsPerSecond, 0) / searchResults.length;
    ...
    if (indexedSearches.length > 0 && linearSearches.length > 0) {
      const indexedAvg =
        indexedSearches.reduce((sum, r) => sum + r.operationsPerSecond, 0) / indexedSearches.length;
      const linearAvg =
        linearSearches.reduce((sum, r) => sum + r.operationsPerSecond, 0) / linearSearches.length;
    ...
    if (highDimResults.length > 0) {
      const avgOps =
        highDimResults.reduce((sum, r) => sum + r.operationsPerSecond, 0) / highDimResults.length;
      if (avgOps < 50) {
        recommendations.push(
