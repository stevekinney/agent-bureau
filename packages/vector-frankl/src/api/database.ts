...
import { debugMethod, withContext } from '@/debug/hooks.ts';
...
import { type QuotaWarning, StorageQuotaMonitor } from '@/storage/quota-monitor.ts';
  ...
  async addBatch(
    ...
    const preparedVectors = await Promise.all(
      vectors.map(async ({ id, vector, metadata }) => {
        ...
        return VectorOperations.prepareForStorage(validatedId, vector, validatedMetadata, {
          normalize: false,
        });
      }),
    );
    ...
    this.quotaWarningListener = async (warning: QuotaWarning) => {
      ...
      if (this.autoEviction && (warning.type === 'critical' || warning.type === 'emergency')) {
        try {
          ...
          log.info(`Attempting automatic eviction to free ${this.formatBytes(targetBytes)}`);
