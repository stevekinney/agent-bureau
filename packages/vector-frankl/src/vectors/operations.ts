...
import type { VectorData, VectorFormat } from '@/core/types.ts';

import { SIMDOperations } from '../simd/simd-operations.ts';
import { WASMOperations } from '../wasm/wasm-operations.ts';
import { VectorFormatHandler } from './formats.ts';
...
export class VectorOperations {
  ...
  static async prepareForStorage(
    ...
    const finalVector = options?.normalize ? await this.normalize(float32Vector) : float32Vector;
