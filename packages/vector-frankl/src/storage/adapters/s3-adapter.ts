...
import { calculateMagnitude, jsonToVectorData, vectorDataToJson } from './serialization.ts';
...
export class S3StorageAdapter implements StorageAdapter {
  ...
  private requireS3(): BunS3Client {
    if (!this.s3) {
      throw new Error('S3StorageAdapter is not initialized. Call init() before using the adapter.');
    }
    return this.s3;
  }
