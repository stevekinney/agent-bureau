...
export class ScalarQuantizer extends BaseCompressor {
   ...
   */
  private quantizeVector(vector: Float32Array, bounds: QuantizationBounds, bits: number): number[] {
    ...
    if (this.scalarConfig.strategy === 'per-dimension' && bounds.dimensionBounds) {
      ...
      for (let i = 0; i < vector.length; i++) {
        ...
        const quantizedValue = quantizeValue(vectorValue, dimBounds.min, dimBounds.max, bits);
  ...
  private packCompressedData(
    ...
    new Uint8Array(fullBuffer, metadataSize, dataSize).set(new Uint8Array(quantizedBuffer));

    // Append per-dimension bounds after quantized data
    if (hasPerDimBounds && bounds.dimensionBounds) {
      const boundsView = new DataView(fullBuffer, metadataSize + dataSize, perDimBoundsSize);
