...
describe('VectorStorage', () => {
  ...
  describe('batch operations', () => {
    ...
    test('should handle partial failures in getMany', async () => {
      const vector = await VectorOperations.prepareForStorage('vec1', new Float32Array([0.1, 0.2]));
      await storage.put(vector);
    ...
    test('should report progress during batch operations', async () => {
      const vectors = await Promise.all(
        Array.from({ length: 100 }, (_, i) =>
          VectorOperations.prepareForStorage(`progress-vec-${i}`, new Float32Array([i * 0.01])),
        ),
      );
  ...
  describe('delete operations', () => {
    ...
    test('should clear all vectors', async () => {
      const vectors = await Promise.all(
        Array.from({ length: 5 }, (_, i) =>
          VectorOperations.prepareForStorage(`clear-vec-${i}`, new Float32Array([i * 0.1])),
        ),
      );
  ...
  describe('utility operations', () => {
    ...
    test('should count vectors', async () => {
      ...
      const vectors = await Promise.all(
        Array.from({ length: 3 }, (_, i) =>
          VectorOperations.prepareForStorage(`count-vec-${i}`, new Float32Array([i * 0.1])),
        ),
      );
    ...
    test('should get all vectors', async () => {
      const vectors = await Promise.all(
        Array.from({ length: 3 }, (_, i) =>
          VectorOperations.prepareForStorage(`all-vec-${i}`, new Float32Array([i * 0.1, i * 0.2]), {
            index: i,
          }),
        ),
      );
