...
import { InvalidFormatError } from '@/core/errors.ts';
import {
...
describe('OptimizedDistanceMetrics', () => {
  describe('euclideanOptimized', () => {
    ...
    it('should handle vectors with length 1 (no unrolled iterations)', () => {
      expect(OptimizedDistanceMetrics.euclideanOptimized(float32(3), float32(7))).toBeCloseTo(4, 5);
    });
...
describe('registerCustomMetric', () => {
  ...
  it('should allow overwriting an existing metric', () => {
    ...
    expect(
      DistanceMetrics.getInstance().get('__overwrite_target').calculate(float32(), float32()),
    ).toBe(1);
