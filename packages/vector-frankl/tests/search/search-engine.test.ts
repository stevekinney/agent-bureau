...
import { DimensionMismatchError } from '@/core/errors.ts';
import type { StorageAdapter, VectorData } from '@/core/types.ts';
import { SearchEngine } from '@/search/search-engine.ts';
import { MemoryStorageAdapter } from '@/storage/adapters/memory-adapter.ts';

/**
 * Build a VectorData object from raw values.
 */
function makeVector(id: string, values: number[], metadata?: Record<string, unknown>): VectorData {
...
describe('SearchEngine', () => {
  ...
  describe('dimension validation', () => {
    ...
    it('should throw DimensionMismatchError on searchRange with wrong dimension', async () => {
      ...
      expect(engine.searchRange(wrongDimension, 0.5)).rejects.toThrow(DimensionMismatchError);
    });
  ...
  describe('search', () => {
    ...
    it('should include metadata when includeMetadata is true', async () => {
      const vectors = [makeVector('tagged', [1, 0, 0], { category: 'science', rating: 5 })];
      const engine = new SearchEngine(await createMockStorage(vectors), 3, 'cosine', {
        useWorkers: false,
      });
  ...
  describe('searchRange', () => {
    ...
    it('should respect maxResults', async () => {
      const vectors = Array.from({ length: 20 }, (_, i) => makeVector(`v${i}`, [1, 0, 0]));
      const engine = new SearchEngine(await createMockStorage(vectors), 3, 'euclidean', {
        useWorkers: false,
      });
  ...
  describe('searchStream', () => {
    it('should yield results in batches', async () => {
      const vectors = Array.from({ length: 15 }, (_, i) => makeVector(`v${i}`, [i + 1, 0, 0]));
      const engine = new SearchEngine(await createMockStorage(vectors), 3, 'euclidean', {
        useWorkers: false,
      });
    ...
    it('should handle maxResults smaller than total', async () => {
      const vectors = Array.from({ length: 10 }, (_, i) => makeVector(`v${i}`, [i + 1, 0, 0]));
      const engine = new SearchEngine(await createMockStorage(vectors), 3, 'euclidean', {
        useWorkers: false,
      });
