  ...
  describe(name, () => {
    ...
    describe('putBatch', () => {
      it('stores all vectors and invokes the progress callback', async () => {
        const vectors = Array.from({ length: 25 }, (_, i) => makeVector(`batch-${i}`, [i, i + 1]));
      ...
      it('stops processing when the abort signal fires', async () => {
        const vectors = Array.from({ length: 50 }, (_, i) => makeVector(`abort-${i}`, [i]));
    ...
    describe('putBatch without options', () => {
      it('stores vectors with no options (default batch size)', async () => {
        const vectors = Array.from({ length: 5 }, (_, i) => makeVector(`no-opts-${i}`, [i]));
