...
describe('createStallWatchdog', () => {
  it('reports unknown reachability and progress before any pulse arrives', () => {
    ...
    // cadenceMs 1000, threshold 3 — zero missed pulses yet, but no pulse
    // recorded means "unknown" progress and "reachable" only once cadence
    // math with 0 missed pulses applies; assert both against the concrete
    // shipped semantics.
    expect(assessment.missedPulseCount).toBe(0);
    watchdog.dispose();
  });

  it('accrues missedPulseCount as ticks pass with no pulse', () => {
    const clock = createManualClock();
  ...
  it('recovers missedPulseCount to zero on the next fresh pulse (resume-on-next-pulse)', () => {
    const clock = createManualClock();
  ...
  it('resumes normal accrual after a suspension gap once ticking normally again', () => {
    ...
    clock.advance(1000); // a normal cadence tick with no new pulse
    expect(watchdog.assess().missedPulseCount).toBe(1);
