...
while any registered child's own `closed()` is still pending — not merely
while the child's `result()` is unresolved. A run with no `childRegistry`, or
one with zero registered children, behaves identically to before this
change, with no added latency. Aborting one child never affects an untouched
sibling's own path to settlement. This gate applies uniformly to both the
in-memory and durable `ActiveRun` paths: `createDurableActiveRun` and
`reattachDurableActiveRun` (`durable/active-run-adapter.ts`) also thread
`RunOptions.childRegistry` through and fold `awaitChildrenClosed()` into
their own `closed()`, so a durable parent's `closed()` waits on its children
exactly the same way an in-memory parent's does — including a child
dispatched after a reattach/recovery, discovered through the same registry.
...
Every generate-function wrapper this package ships preserves whatever descriptors the function it wraps carried, so composing wrappers around a described provider factory never silently downgrades an Agent's profile to `'opaque'`: `withStreaming`, `withRoutingMetrics`, `withEnhancedStreaming`, `withCache`, `withCacheMetrics`, the provider instrumentation wrapper (`instrument`), `createFallbackGenerate`, and `createFalloverGenerate` all read `readBackendDescriptors` off their inner function(s) and reattach the result to the function they return. `createFallbackGenerate` and `createFalloverGenerate`, like `createRoutingGenerate`, attach the ordered union of every candidate's descriptors, deduplicated by `(provider, endpoint, model)`.
...
| `execute_tool {name}` (`armorer/instrumentation`)                                        | Execute tool                                   | `execute_tool {gen_ai.tool.name}` | INTERNAL | `gen_ai.operation.name=execute_tool`, `gen_ai.tool.name`, `gen_ai.tool.call.id`, `gen_ai.tool.description`, `error.type`                                          | `gen_ai.tool.call.arguments`/`gen_ai.tool.call.result` are Opt-In under the conventions and deliberately not emitted — they can carry privileged tool arguments/results (AB-87/AB-230). `armorer.tool.*` carries duration/digest/status extensions the spec doesn't define.                                                                                                                                                                                                       |
...
> [!NOTE] Durable event history types (AB-91's `ab91-01` slice, AB-310; widened by AB-320)
> `@lostgradient/operative/durable` also exports `DurableEventOwnerKind` (`'run' | 'session' | 'schedule'` — `'schedule'` added by AB-320, owning a schedule's four definition events only; a scheduled fire stays on the `'run'` owner), `DurableEventOwner`, `DurableEventEnvelope`, `DurableEventPage`, and `DurableEventGap` — the wire types Bureau's `durable-event-history.ts` store and `Bureau.eventHistory(owner, options?)` read/return. `DurableEventEnvelope` is a redacted, owner-scoped projection of Weft's own `FleetEventEnvelope`, carrying a new `schemaVersion` field Weft's envelope does not have. `DurableEventGap` is returned instead of a `DurableEventPage` when a caller's `since` cursor predates the store's retention floor — distinguishable from an ordinary empty page.
...
| `runReactiveSourceConformanceSuite(options, testRunner?)` | AB-92/AB-258's framework-neutral reactive-source conformance suite: registers one `describe` block with one `it` per case (`stableSnapshotIdentity`, `immutableReplacementAfterChange`, `multipleIndependentSubscribers`, `subscribeReadRaceClosure`, `earlyCompletionBeforeSubscription`, `subscribeUnsubscribeSubscribeNoDuplicateWork`, and — only when `options.reattach` is supplied — `serializableLocatorRoundTrip`) against any subject implementing `ReactiveSourceSubject<TSnapshot>`. Never imports a resource type from this package outside its own generic parameter, so it runs against `ActiveRun`/`AgentRun`'s upcoming snapshot surface or any other conforming resource by supplying an adapter. `testRunner` defaults to `bun:test`'s real `describe`/`it`. |

**Exported types:** `RunRecorder`, `EventRecorder`, `CausalTraceEntry`, `EventRecorderOwnerIdentity`, `ManualRuntimeServices`, `ResourceScope`, `QuiescenceReport`, `LeakedResource`, `LeakedResourceKind`, `LeakedResourceDiscoveredVia`, `DetachedResource`, `RegisterableResource`, `ClosableRun`, `ReactiveSourceSubject`, `ReactiveSourceConformanceOptions`, `ReactiveSourceConformanceTestRunner`.
