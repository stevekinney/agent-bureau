...
| `steering`            | `SteeringGate`                                         | AB-67 runtime steering: `{ getDesiredState(), awaitResume() }`, consulted at the entry of every step. `paused: true` blocks the step until `awaitResume()` resolves or `signal` fires. Optional — omit for unsteered runs. |
