...
- 7076e8d: Enforce `ReusableApprovalGrant.scope` in grant matching (AB-364, AB-46, AB-346).
...
- 35f62f1: Compose the `RuntimeServices` injectable runtime-service seam into `createToolbox` and `ExecutionLifecycle`, and move `nextExecutionId` off module scope (AB-92, delivered by AB-254).
...
- 485ad0b: Match reusable approval grants ahead of capability approval, and add `Toolbox.issueGrant`/`revokeGrant`/`listGrants` (AB-46, AB-346; builds on AB-345's grant type and `GrantStateStore`).
...
- 1c740c9: Add the reusable-approval-grant type and `GrantStateStore` to `packages/armorer/src/approval-binding.ts` (AB-46, implemented by AB-345).
...
- 4fbe64c: Closes the six armorer public `Tool`/`ToolConfiguration` type-vs-runtime gaps `packages/armorer/test` worked around with documented casts after AB-295 widened the `check-types` gate (AB-308).
  ...
  - `stableStringify()` (`src/core/loop-detection.ts`) is a runtime-defect fix, not a type widening: it used to return the runtime value `undefined` (via `JSON.stringify`) for a top-level `undefined`, function, or symbol despite its declared `string` return type, and threw for a `BigInt`. It now mirrors `JSON.stringify`'s own treatment of those three "absent" types — `'null'` at the top level or inside an array, omitted as an object property — and stringifies a `BigInt` to its decimal text instead of throwing, so every caller, including the tool-call-argument hash function on the loop-detection path, always gets a real string back.
...
- 17b2818: Fix `combineToolboxes` silently dropping the first toolbox's approval configuration (AB-362). `combineToolboxes(...)` rebuilt a fresh toolbox forwarding only the shallow-merged `context`, so the first toolbox's `policy` (including any `needs_approval` `beforeExecute` hook), `approvalPolicy`, `approvalSecret`, `approvalStateStore`, and `grantStateStore` — along with the rest of its `ToolboxOptions` — were all lost. Any caller combining a toolbox that gates calls behind approval with additional tools (for example, Bureau's `wireDurableOptInTools`, which grafts the durable `requestHumanInput`/`scheduleWakeup` tools onto a run's toolbox) got a combined toolbox that executed every gated tool call immediately, with no pending review ever created and no reusable-approval-grant matching in effect — a live security-relevant regression.

  `combineToolboxes` now forwards the first toolbox's approval- and toolbox-identity-related options — `policy`, `policyContext`, `approvalPolicy`, `approvalSecret`, `approvalStateStore`, `grantStateStore`, `approvalBindingTtlMs`, `approvalNow`, `approvalNonce`, `policyRevision`, `approvalRevision`, `toolboxRevision`, `readOnly`, `allowMutation`, and `allowDangerous` — into the combined toolbox, the same way `Toolbox.extend()` already forwards its own options into an extended toolbox. This is a narrow, explicit allowlist rather than every option `ToolboxOptions` carries: forwarding `middleware` would apply it a second time to configurations `toJSON()` already transformed with it, and forwarding `signal` would tie the combined toolbox's abort listener to a signal it never gets a chance to detach from on normal completion. `policyContext` travels alongside `policy` because a registry-level `beforeExecute` hook that branches on `policyContext` (a tenant flag, an approval-context provider) would otherwise run with that context missing, silently defaulting to allow when the hook expects a value to be present. Combining multiple toolboxes is not a merge of approval configuration across them: only the first toolbox's approval gating governs calls to the combined toolbox, exactly as `extend()` already behaves for the toolbox `extend()` is called on. The forwarding channel keeps `approvalSecret` out of the combined toolbox's public surface entirely — it is never a method or property on `Toolbox`, so no caller holding a toolbox reference can read it back off.

- 794c74d: Fix `buildDefaultTool` (`packages/armorer/src/create-toolbox.ts`) dropping `progress` and `dispatch` off the runtime tool context handed to a tool body registered on a toolbox as a raw configuration (as opposed to an already-built `createTool(...)` `Tool`). Previously `context.progress(...)` and `context.dispatch(...)` silently no-opped inside such a tool's body, so no consumer of `tool.progress` — operative's curated bubble, the gateway live stream, or AB-88's liveness — ever saw progress reported by a real tool executed through a toolbox.
...
- ca96bd6: `searchTools`'s `@deprecated` tag previously sat on only the first of its six overload signatures, so only the zero-argument call resolved to a tagged signature and `@typescript-eslint/no-deprecated` warned inconsistently — one call flagged, roughly a dozen sibling calls to the exact same deprecated function silently clean. All six overload signatures (the five public overloads plus the implementation signature) now carry the identical "Use `queryTools` for tool discovery" `@deprecated` tag, so every call to `searchTools` produces the same compiler hint regardless of which overload it resolves to. This is a documentation-only change: no runtime behavior, parameter, or return type changed.
- 2a5bcbe: Migrates armorer's pre-existing `@typescript-eslint/no-deprecated` warnings (AB-367, bucket 4 of AB-366's pull request #572) to their non-deprecated replacements:
