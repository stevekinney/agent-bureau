...
This package lives in the `agent-bureau` monorepo under `packages/armorer/`. Tasks are orchestrated via Turborepo from the monorepo root.
...
```bash
turbo run build --filter=armorer        # Build this package
turbo run test --filter=armorer         # Run tests
...
- **ESM + TypeScript**: Source files are TypeScript modules; build output is ESM + CJS.
...
- **ESLint**: Flat configuration with `typescript-eslint` presets; type-aware rules only under `src/**` for speed.
