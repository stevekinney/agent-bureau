...
import type { MemoryRecordStorage } from '../src/memory-record-storage';
...
describe('getMemoryStatus', () => {
  ...
  it('returns correct total entry count', async () => {
    ...
    const status = await getMemoryStatus(storage, { namespaces: ['default'] });
  ...
  it('reports storage type from constructor name', async () => {
    const status = await getMemoryStatus(storage, { namespaces: [] });
  ...
  it('reports custom storage type when provided', async () => {
    const status = await getMemoryStatus(storage, { namespaces: [], storageType: 'SQLite' });
  ...
  it('reports embedding cache size when a CachedEmbedder is provided', async () => {
    ...
    const status = await getMemoryStatus(storage, { namespaces: [], embedder: cachedEmbedder });
  ...
  it('does not report cache size for a non-cached embedder', async () => {
    ...
    const status = await getMemoryStatus(storage, { namespaces: [], embedder });
